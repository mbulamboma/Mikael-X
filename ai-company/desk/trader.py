...
du debat Bull/Bear, plus le regime, le classement des strategies, le bilan chiffre et les
lecons. Il decide : pour chaque candidat, ouvrir (a des niveaux techniques REELS, R:R >= 2,
stop a un vrai niveau de structure) ou s'abstenir. Il ne calcule JAMAIS le lot (moteur FTMO).

Il ne gere pas le book (c'est le Trade Manager) : il ne rend que des actions `open`. Ses
ouvertures passent ensuite par le Risk Manager (qui peut durcir) puis par le moteur FTMO
deterministe (sizing + veto). Il APPREND de ses erreurs ET de celles des autres agents
(lecons taguees, bilan) : tant qu'un defaut recurrent est liste, le corriger prime.
...
- APPRENTISSAGE : corrige d'abord les DEFAUTS RECURRENTS du bilan et respecte tes lecons +
  celles des autres agents. La confiance n'est pas un edge : exige la preuve technique.
...
{{"analyse": "2-3 phrases: regime, biais, ce que tu fais et pourquoi",
  "actions": [
    {{"type": "open", "strategy": "trend_follow", "symbol": "EURUSD", "direction": "buy",
      ...
      "rationale": "macro + technique + ce qui invaliderait la these"}}
  ]}}
...
class Trader(DeskAgent):
    ...
    def decide(self, summary: dict, mandate: dict, briefs: dict, debate: dict,
               lessons: str, memoire: dict | None = None) -> list[dict]:
        ...
        blocs.append("== TES LECONS (et celles des autres agents) ==\n" + (lessons or "(aucune)"))
